...
**不想碰终端？** 页面右上角绿色 **Code** 按钮 → **Download ZIP**，下载下来双击解压，打开解压出来的文件夹，里面有个叫 **`双击我安装.command`** 的文件——文件名就是操作说明，找到它点开就行。它会自动做完下面第 1-3 步（安装、导入两个示例 Skill、激活 core 场景包），最后打开浏览器面板，从此往后都在浏览器里点。首次双击如果 macOS 提示"来自身份不明的开发者，无法打开"，在这个文件上按住 Control 点一下（或右键）→ 选"打开"→ 再确认一次"打开"，只有第一次需要这样，是系统对所有下载脚本的统一提示，不代表文件有问题。想更精细地控制每一步、或者不是 macOS 图形界面环境，才需要往下看命令行版本。
...
自包含的 HTML 面板：顶部统计卡片、Skill 列表（支持勾选批量操作、按分类筛选、按分类分组排序）、场景包成员、工具接入状态、日间/夜间/跟随系统主题切换。两种模式二选一，细节见后面的[「面板：静态快照 vs 本地服务」](#面板静态快照-vs-本地服务)。改变激活/连接状态的命令执行后会自动重建静态快照，一般不用手动跑 `dashboard build`。
...
`import-github` 复用 `import` 的同一套安全检查（哈希去重、`--replace` 先把旧版本挪进废纸篓、`--activate`），多加一步：clone 到临时目录、定位 `SKILL.md`、给一段来源预览（声明的 name/description、带没带可执行脚本）供你确认后再落地。仓库根目录没有 `SKILL.md` 时会自动找出仓库里所有真正挂着 `SKILL.md` 的目录（不管套了几层分类）列成候选，提示你用 `--path` 指定；链接里混进的零宽空格、全角标点会被自动规整，不会因为这类不可见字符直接报"链接格式不对"或悄悄拼错 owner/repo。URL 除了完整的 `https://github.com/owner/repo`，也认不带协议头的 `owner/repo` 简写（面板向导和命令行都支持）。
...
- **从 GitHub 导入**：输入仓库 URL（也认 `.../tree/<ref>/<子路径>` 这种浏览器地址栏形式，自动拆出 ref 和子路径；也认不带协议头的 `owner/repo` 简写），先克隆到临时目录做只读预览（声明的 name/description、带没带可执行脚本），确认后才真正落地；仓库里有多个 Skill 时会列出所有候选路径，支持勾选多个一次性批量导入，导入中的每一项独立显示进度。
