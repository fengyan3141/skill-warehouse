...
**不想碰终端？** 页面右上角绿色 **Code** 按钮 → **Download ZIP**，下载下来双击解压，打开解压出来的文件夹，双击里面的 `install.command`——它会自动做完下面第 1-3 步（安装、导入两个示例 Skill、激活 core 场景包），最后打开浏览器面板，从此往后都在浏览器里点。首次双击如果 macOS 提示"来自身份不明的开发者，无法打开"，在这个文件上按住 Control 点一下（或右键）→ 选"打开"→ 再确认一次"打开"，只有第一次需要这样，是系统对所有下载脚本的统一提示，不代表文件有问题。想更精细地控制每一步、或者不是 macOS 图形界面环境，才需要往下看命令行版本。
